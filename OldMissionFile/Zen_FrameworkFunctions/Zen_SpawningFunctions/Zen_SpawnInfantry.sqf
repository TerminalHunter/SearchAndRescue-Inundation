...
private ["_spawnPos", "_side", "_skill", "_group", "_soldierlistrand", "_unitCount", "_spawnNumber", "_soldierlist", "_menType", "_faction", "_filterTypes", "_i", "_DLC"];
...
if (typeName _spawnNumber != "ARRAY") then {
    _unitCount = _spawnNumber;
} else {
    if (count _spawnNumber == 2) then {
        _unitCount = [(_spawnNumber select 0), (_spawnNumber select 1), true] call Zen_FindInRange;
    } else {
        _unitCount = random _spawnNumber;
    };
};
