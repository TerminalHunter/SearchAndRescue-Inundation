...
_objects = [];
{
    ...
    _enableSim pushBack (_x select 7);
    ...
    if ((_x select 0) == "Zen_Template_Object_Type_Simple") then {
        _obj setDamage (_x select 6);
        _obj allowDamage false;
    } else {
        if ((_x select 0) == "Zen_Template_Object_Type_WeaponHolder") then {
            _obj setDamage (_x select 6);
            _obj allowDamage false;
            0 = [_obj, (_x select 8)] call Zen_GiveLoadoutCustom;
        } else {
            {
            ...
            _obj setFuel (_x select 9);
            0 = [_obj, (_x select 8)] call Zen_GiveLoadoutCustom;
            
            _data = _x;
            {
                _obj animate [_x, (_data select 10) select _forEachIndex];
    ...
    {
        0 = [_x, _obj] call Zen_InvokeAction;
    } forEach (_x select 5);
