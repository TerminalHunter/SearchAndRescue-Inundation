...
if (typeName (_this select 0) == "SCALAR") then {
    ...
    if ((typeName (_this select 0) == "ARRAY") && {(typeName ((_this select 0) select 1) == "STRING")}) then {
        _pos = [(_area select 0)] call Zen_ConvertToPosition;
    } else {
...
if (typeName _area == "STRING") then {
    ...
    if (count _vars > 0) then {
        if ((typeName (_this select 0) == "ARRAY") && {(typeName ((_this select 0) select 1) == "STRING")}) then {
            _pos = _vars call Zen_CalculatePositionLine;
        } else {
            _pos = _vars call Zen_CalculatePositionObject;
        };
    };
