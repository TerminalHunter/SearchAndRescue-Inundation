...
if (count _this > 1) then {
    if ((typeName _area == "ARRAY") && {(typeName (_area select 1) == "STRING")}) then {
        _min = ((_this select 1) select 0);
        _max = ((_this select 1) select 1);
        _area set [0, [(_area select 0)] call Zen_ConvertToPosition];
    } else {
...
(if (typeName _area == "STRING") then {
    ...
    (if ((typeName _area == "ARRAY") && {(typeName (_area select 1) == "STRING")}) then {
        ([_area, _min, _max, _allBlacklist, _allWhitelist, _oneWhitelist, _water, _roadID, _roadDist, _minAngle, _maxAngle,_objSwitch,_objLimit,_objDist,_pointAvoidSwitch,_pointAvoidArray,_pointAvoidDist,_nearWaterSwitch,_nearWaterDist, _terrainSlopeSwitch, _terrainSlopeAngle, _terrainSlopeRadius, _ambientClutterSwitch, _ambientClutterCount, _ambientClutterRadius, _heightSwitch, _heightNumber, _heightRadius, _maxFailures])
    } else {
