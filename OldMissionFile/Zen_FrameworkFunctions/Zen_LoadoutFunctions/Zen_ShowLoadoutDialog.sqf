...
private ["_object", "_index", "_kitArray", "_user", "_actionID"];

if !([_this, [["OBJECT"], ["OBJECT"], ["STRING"]], [], 3] call Zen_CheckArguments) exitWith {
    call Zen_StackRemove;
};

_object = _this select 0;
_user = _this select 1;
_dialogID = _this select 2;
_actionID = _this select 3;

player setVariable ["Zen_LoadoutsDialogObject", _object];
player setVariable ["Zen_LoadoutsDialogAction", _actionID];
