...
private ["_kit", "_unit", "_object", "_actionID", "_index", "_maxCount", "_curCount", "_loadoutData"];

_kit = _this select 0;
_object = player getVariable "Zen_LoadoutsDialogObject";
_unit = player getVariable "Zen_LoadoutsDialogUser";
_actionID = player getVariable "Zen_LoadoutsDialogAction";
...
_index = ([Zen_Loadout_Action_Array_Local, _actionID, 0] call Zen_ArrayGetNestedIndex) select 0;
...
if ((_maxCount != -1) && {_curCount >= _maxCount}) then {
    0 = [_actionID, [_object]] call Zen_DeleteActionClient;
    0 = [Zen_Loadout_Action_Array_Local, _index] call Zen_ArrayRemoveIndex;
} else {
    Zen_Loadout_Action_Array_Local set [_index, [_actionID, _maxCount, _curCount]];
};
