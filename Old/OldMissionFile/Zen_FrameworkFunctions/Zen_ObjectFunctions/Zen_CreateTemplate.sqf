...
_center set [2, (getPosATL ([_objResult, {-((getPosATL _this) select 2)}] call Zen_ArrayFindExtremum)) select 2];
{
    if (_x isKindOf "static" ||_x isKindOf "building" || _x isKindOf "thing") then {
        _templateData pushBack (["Zen_Template_Object_Type_Simple", typeOf _x, (getPosATL _x) vectorDiff _center, getDir _x, [vectorDir _x, vectorUp _x], ([_x] call Zen_GetObjectActions), getDammage _x, simulationEnabled _x]);
    } else {
        if (_x isKindOf "WeaponHolder") then {
            _loadoutData = [_x] call Zen_GetUnitLoadout;
            _loadout = [_loadoutData] call Zen_CreateLoadout;
            _templateData pushBack (["Zen_Template_Object_Type_WeaponHolder", typeOf _x, (getPosATL _x) vectorDiff _center, getDir _x, [vectorDir _x, vectorUp _x], ([_x] call Zen_GetObjectActions), getDammage _x, simulationEnabled _x, _loadout]);
        } else {
            ...
            _templateData pushBack (["Zen_Template_Object_Type_Vehicle", typeOf _x, (getPosATL _x) vectorDiff _center, getDir _x, [vectorDir _x, vectorUp _x], ([_x] call Zen_GetObjectActions), _damage, simulationEnabled _x, _loadout, fuel _x, _anims]);
        };
    };
} forEach _objResult;
...
["template_name"
    [
        ["object type simple", "object classname", vector difference from center, vector direction, vector up, [actions], damage, sim]
        ["object type weapon", "object classname", vector difference from center, vector direction, vector up, [actions], damage, loadout, sim]
        ["object type vehicle", "object classname", vector difference from center, vector direction, vector up, [actions], damage selections, sim, "loadout", fuel, animation phases]
        ...
    ]
]
//*/