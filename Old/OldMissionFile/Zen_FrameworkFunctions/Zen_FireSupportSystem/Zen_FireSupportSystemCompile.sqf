...
Zen_AddFireSupportAction_ShowDialog_MP = {
    ...
    0 = [(Zen_Fire_Support_Action_Dialog_Data select 3), ["MapPosition", (getPosATL player) vectorAdd [random 1, 0, 0]]] call Zen_UpdateControl;
...
Zen_AddFireSupportAction_DialogOK_MP = {
    ...
    if (count _indexes > 0) then {
        ...
        if (_isCustom) then {
            ...
            // openMap true;
            hint "Hold Left-click on the desired fire support target.";
            Zen_Fire_Support_Target_Pos_Local = [];
